...
source("./R/PromoAnalysis//PromotionEffectCalculator.R")
...
for(file in files) {
  ...
  ts1 = ts(data = t2$LOG_UNITS,start = c(startYear,startMonth)
           ,frequency = 12)
  ts3 = ts(data = t2$UNITS,start = c(startYear,startMonth)
           ,frequency = 12)
  reg.matrix = as.matrix(t2[,c("LOG_PRICE","FEATURE"
                               ,"DISPLAY","TPR_ONLY")])
  reg.matrix2 = as.matrix(t2[,c("PRICE","FEATURE"
                                ,"DISPLAY","TPR_ONLY")])
  model = auto.arima(x = ts3,xreg =  reg.matrix2)
  print(summary(model))
  
  h= 12
  f = forecast.Arima(object = model,xreg = reg.matrix2[1:h,])
  plot(f)
  r = calc.promotion.effect(ts3,reg.matrix2)
  ...
  #f = forecast.Arima(object = model,xreg = reg.matrix[1:h,])
  #plot(f)
  #mean = exp(f$mean)
  #lower = exp(f$lower[,1])
  #upper = exp(f$upper[,1])
